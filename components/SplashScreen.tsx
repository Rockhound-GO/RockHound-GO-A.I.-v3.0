...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onFinish }) => {
  ...
  useEffect(() => {
    // 1. Progress Bar Animation - Faster (2.5s total approx)
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        return prev + 2; // Double speed
      });
    }, 30); // Faster tick

    // 2. Boot Text Sequence - Faster
    let logIndex = 0;
    ...
    }, 400); // Faster log appearance

    // 3. Finish - Total duration ~2.5s
  ...
  return (
    <div 
      ref={containerRef}
      className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center overflow-hidden font-mono transition-all duration-500 ease-in-out"
    >
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-md px-6">
        ...
        <div className="flex justify-between w-full text-[10px] text-cyan-500/60 font-bold mb-6">
            <span>LOADING MODULES</span>
            <span>{progress}%</span>
        </div>

        {/* Boot Logs - Fixed Height to prevent layout shift */}
